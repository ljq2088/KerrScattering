...
from src.teukolsky_scalar import KerrParams, teukolsky_lambda_s0
...
def _collocation_residuals(data, N, z_left, z_right, branch, boundary,
    ...
    scale = np.maximum(np.abs(B2 * u_zz) + np.abs(B1 * u_z) + np.abs(B0 * u), 1e-300)
    ...
    return z[keep], residual[keep], rel[keep], np.nonzero(keep)[0]
...
def run_spectral_quality(args):
    ...
    for branch, subdomain, z_left, z_right, boundary, N, kappa in branch_configs:
        ...
        z, residual, rel, residual_index = _collocation_residuals(
            data,
            N,
            z_left,
            z_right,
        ...
        for idx, zi, ri, rrel in zip(residual_index, z, residual, rel):
...
def main(argv=None):
    ...
    _write_csv(
        ...
        ["branch", "subdomain", "node_index", "z", "abs_residual",
         "relative_residual", "real_residual", "imag_residual"],
    )
    _write_csv(
        args.summary_output,
        summary_rows,
        ["case", "l", "m", "a", "omega", "N", "branch", "subdomain",
         ...
         "p95_relative_residual", "median_relative_residual", "condition"],
    )
