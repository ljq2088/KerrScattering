...
\begin{abstract}
We study weakly nonlinear scattering of a complex scalar field from a Kerr
black hole with a cubic self-interaction.  We solve the spin-weight-zero
Teukolsky equation with an endpoint-inclusive, two-domain Chebyshev method and
construct the first-order Green-function response.  Nonlinear transmission and
reflection coefficients are defined as derivatives of the horizon and infinity
energy fluxes at fixed incident flux.  Rotation enters through the
co-rotating horizon frequency, spheroidal harmonics, and the $\Sigma$-weighted
source projection.

For an axisymmetric sample with $a=0.5$ and $\ell=0,1,2$, the accepted spectra
show broad Breit-Wigner-type features whose fitted centers lie on the same
frequency scale as the real parts of the corresponding scalar Kerr
quasinormal frequencies.  The high-frequency
data are described by finite-window exponential fits with effective
temperatures within $3.2$--$13.6\%$ of the Kerr Hawking temperature.  The
low-frequency transmission coefficients follow a centrifugal-barrier
hierarchy, while the reflected first-order coefficient becomes cancellation
limited at the lowest frequencies.  Spectral tails, field-dependent radial
residuals, Wronskian constancy, resolution scans, and flux balance are used
together as internal accuracy tests; the accepted first-order balance
residuals are at or below $4.3\times10^{-12}$.  A bounded $\ell=m=2$ scan
resolves the signed horizon-flux change across the superradiant threshold.

This is a prescribed scalar response on a fixed Kerr background.  It is not a
second-order metric perturbation, a gravitational self-force calculation, or an
EMRI waveform model.
\end{abstract}
...
\cite{Wei2025,Berens2024}.  Recent effective-source constructions make the
distinction concrete for both generic first-order orbits and second-order
quasicircular calculations: the residual equation carries puncture data,
worldline information, and slow-time source terms that are absent from the
present stationary scalar problem~\cite{ZhangEffectiveSource2025,
UptonWardell2026}.

The distinction is also important when comparing this scalar prototype with
nonlinear Teukolsky calculations.  In the gravitational problem the
second-order source contains tetrad and metric-perturbation terms and must be
combined with gauge completion and, for self-force applications, a
singular-field subtraction~\cite{GreenHollandsZimmerman,SpiersPoundMoxon,
SpiersSource2024}.  Our source is instead a smooth local cubic functional of
one prescribed scalar mode.  We therefore use the EMRI and second-order
literature to delimit the interpretation of the calculation, not to identify
the scalar coefficient with a gravitational self-force observable.
...
The contributions of this paper are threefold.  First, we derive the Kerr
fixed-incident normalization of the weak cubic response, including the horizon
factor $p$ and the $\Sigma$-weighted spheroidal source projection, so that the
reported quantities are energy-flux derivatives rather than unnormalized Green
amplitudes.  Second, we implement the endpoint-inclusive two-domain
Chebyshev construction and combine spectral tails, field-dependent radial
residuals, Wronskian constancy, and flux balance into an explicit acceptance
protocol.  Third, we use that protocol to separate robust axisymmetric
resonance and tail trends from cancellation-limited low-frequency reflected
coefficients, while exposing the off-diagonal target-channel structure and a
bounded superradiant sign change.  The resulting claims are deliberately
restricted to the prescribed scalar response problem defined above.
...
in the text; the ranges use three, four, and five points.  Here the superscript
$(0)$ labels the linear transmission coefficient and $(1)$ labels the
first-order nonlinear coefficient.  The last two columns give the largest
log-space RMS residual over those fits.}
\label{tab:low-window}
\begin{ruledtabular}
\begin{tabular}{c c c c c c}
$\ell$ & baseline $(\alpha_\ell^{(0)},\alpha_\ell^{(1)})$
& range of $\alpha_\ell^{(0)}$ & range of $\alpha_\ell^{(1)}$
& $\max\sigma_{\log}^{(0)}$ & $\max\sigma_{\log}^{(1)}$\\
\hline
...
Second, the peaks of $T_\ell^{(1)}$ lie on the frequency scale set by the real
...
$7.1\times10^{-5}$, $3.2\times10^{-4}$, and $1.3\times10^{-5}$ for
$\ell=0,1,2$, respectively, whereas the corresponding half-widths vary by
$5.3\times10^{-3}$, $8.8\times10^{-3}$, and $4.5\times10^{-4}$.  The
...
Green-function calculation unchanged.  The linear solution is normalized at
...
each $\ell$, with the peak position lying on the frequency scale of the corresponding
...
Tables~\ref{tab:low-window}, \ref{tab:fit}, \ref{tab:tail-window},
\ref{tab:numerics}, \ref{tab:production-convergence}, \ref{tab:convergence},
\ref{tab:channels}, \ref{tab:superradiant}, and \ref{tab:spin-limit} are
...
experimental data are used.  In the CSV files, ``status=ok'' records a
completed solver call; the manuscript acceptance decision is a separate
postprocessing predicate on the recorded absolute first-order balance
residual, $|T^{(1)}+R^{(1)}|$.
